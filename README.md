...
- A monomial $\alpha_e \prod_{i \in e} x_i$ is described with the line $\alpha_e x_{i_1} \dots x_{i_k}$ where $e = \{x_{i_1}, \dots, x_{i_k}\}$. 
...
Several examples can be found in the directory `examples/poly`. The script `tools/convert.py` takes a file representing a polynomial and convert it into several formats. It prints the output on the standard output. The precise usage is:
...
Once converted into a weighted CNF formula, the BPO problem can be solved using standard tools from propositional logic theory. The simplest approach is to find the smallest value using a branching algorithm, where variables are branched on a value on after the other. When a model is found, its weight is reported and we backtrack to the last decision. If done naively, this algorithm boils down to enumerating every model of the formula and is not efficient. We can add many interesting improvement to this naive algorithm:

- Keep a cache of formulas already solved because sometimes, the same formula appears under distinct partial assignments.
- Detect, at each step, when the formula is made of more than one connected components, which can be maximized independently.
- Use a SAT solver to avoid visiting unsatisfiable branches... 

This algorithm known as exhaustive DPLL actually builds a factorized representation of every model of the Boolean formula, known as a decision-DNNF circuit (decision Decomposable Normal Form). This representation can in turn be used to find the best assignment and even an extended formulation of the multilinear polytop. Check the paper for details.
...
This script outputs on the standard outputs a json representation of a DNNF computing the multilinear polytop of the input polynomial. This json representation may be used to:
...
- Compute an extended formulation of the multilinear polytop of size linear in the circuit using `extended.py` (**todo**),
